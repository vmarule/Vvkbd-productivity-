...
function repairInvalidTimes(tasks){let changed=false;tasks.forEach(t=>{if(!t||!t.start||!t.end)return;const s=localParse(t.start),e=localParse(t.end);if(isNaN(s)||isNaN(e))return;if(e<=s){const fixed=new Date(s.getTime()+60*60000);t.end=makeDT(fixed,fixed.getHours(),fixed.getMinutes(),fixed.getSeconds());t._dateTimeRepairedAt=new Date().toISOString();changed=true}});return changed}
function materialise(){
 const d=load(),del=deleted();
 d.tasks=d.tasks.filter(t=>!del.has(String(t.id)));
 repairInvalidTimes(d.tasks);
...
function installTools(){const more=document.getElementById('more');if(!more||document.getElementById('vvkbdMaintenanceTools'))return;const card=document.createElement('div');card.id='vvkbdMaintenanceTools';card.className='card';card.innerHTML='<h2>Data & reminders</h2><p class="muted">Back up your local data and enable phone reminders.</p><div class="row"><button class="light" id="vvkbdExport">Export backup</button><button class="light" id="vvkbdImport">Import backup</button></div><input id="vvkbdBackupFile" type="file" accept="application/json" style="display:none"><button class="primary" id="vvkbdNotify" style="margin-top:8px;width:100%">Enable phone reminders</button><div id="vvkbdNotifyStatus" class="muted" style="margin-top:7px"></div>';
 ...
 document.getElementById('vvkbdExport').onclick=exportBackup;document.getElementById('vvkbdImport').onclick=()=>document.getElementById('vvkbdBackupFile').click();document.getElementById('vvkbdBackupFile').onchange=e=>{if(e.target.files[0])importBackup(e.target.files[0])};
 const status=document.getElementById('vvkbdNotifyStatus'),update=()=>{status.textContent=('Notification' in window)?(Notification.permission==='granted'?'Phone reminders enabled while the app is running.':'Tap to enable phone reminders.'):'This browser does not support notifications.'};update();
