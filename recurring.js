...
const PRAYER_CUTOFF='2026-08-31';
...
function materialise(){
 ...
 // Prayer is a temporary schedule and must never extend beyond 31 August 2026.
 ...
 for(const master of prayerMasters){
   if(master.repeat&&master.repeat!=='None'){
     const first=dateKey(new Date(master.start));
     d.tasks=d.tasks.filter(t=>!(t.masterId===master.id&&dateKey(new Date(t.start))>PRAYER_CUTOFF));
     if(first>PRAYER_CUTOFF){master.repeat='None';changed=true;continue}
   }
 }
 ...
 for(const master of masters){
   ...
   for(let i=0;i<90;i++){
     ...
     if(isPrayer(master)&&k>PRAYER_CUTOFF)break;
