...
function exportBackup(){const raw=localStorage.getItem(KEY)||'{}';let data;try{data=JSON.parse(raw)}catch(e){data={tasks:[],goals:[],sales:{target:5000}}}const payload={version:1,exportedAt:new Date().toISOString(),data};const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}));a.download='vvkbd-productivity-backup-'+dateKey(new Date())+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
function importBackup(file){const r=new FileReader();r.onload=()=>{try{const p=JSON.parse(r.result),data=p&&p.data?p.data:p;if(!data||!Array.isArray(data.tasks))throw Error('Invalid backup');data.goals=Array.isArray(data.goals)?data.goals:[];data.sales=data.sales||{};if(!Number.isFinite(Number(data.sales.target))||Number(data.sales.target)<=0)data.sales.target=5000;localStorage.setItem(KEY,JSON.stringify(data));materialise();location.reload()}catch(e){alert('Backup could not be imported. Please choose a valid VVKBD JSON backup.')}};r.readAsText(file)}
function installTools(){const more=document.getElementById('more');if(!more||document.getElementById('vvkbdMaintenanceTools'))return;const card=document.createElement('div');card.id='vvkbdMaintenanceTools';card.className='card';card.innerHTML='<h2>Data & reminders</h2><p class="muted">Back up your local data and enable phone reminders.</p><div class="row"><button class="light" id="vvkbdExport">Export backup</button><button class="light" id="vvkbdImport">Import backup</button></div><input id="vvkbdBackupFile" type="file" accept="application/json" style="display:none"><button class="primary" id="vvkbdNotify" style="margin-top:8px;width:100%">Enable phone reminders</button><div id="vvkbdNotifyStatus" class="muted" style="margin-top:7px"></div>';more.appendChild(card);document.getElementById('vvkbdExport').onclick=exportBackup;document.getElementById('vvkbdImport').onclick=()=>document.getElementById('vvkbdBackupFile').click();document.getElementById('vvkbdBackupFile').onchange=e=>{if(e.target.files[0])importBackup(e.target.files[0])};const status=document.getElementById('vvkbdNotifyStatus');const update=()=>{status.textContent=('Notification' in window)?(Notification.permission==='granted'?'Phone reminders enabled.':'Tap to enable phone reminders.'):'This browser does not support notifications.'};update();document.getElementById('vvkbdNotify').onclick=async()=>{if(!('Notification' in window)){update();return}try{const p=await Notification.requestPermission();update();if(p==='granted')scheduleNotifications()}catch(e){status.textContent='Notification permission could not be enabled.'}}}
function notify(title,body,tag){if(!('Notification' in window)||Notification.permission!=='granted')return Promise.resolve();const opts={body,tag,data:{taskTag:tag}};return navigator.serviceWorker&&navigator.serviceWorker.ready?navigator.serviceWorker.ready.then(r=>r.showNotification(title,opts)).catch(()=>{try{new Notification(title,opts)}catch(e){}}):Promise.resolve().then(()=>{try{new Notification(title,opts)}catch(e){}})}
function scheduleNotifications(){if(!('Notification' in window)||Notification.permission!=='granted')return;const d=load(),now=Date.now(),seenKey='vvkbd_notified_reminders_'+dateKey(new Date()),seen=new Set();try{JSON.parse(localStorage.getItem(seenKey)||'[]').forEach(x=>seen.add(String(x)))}catch(e){}d.tasks.forEach(t=>{const mins=Number(t.reminder);if(!Number.isFinite(mins)||mins<0)return;const start=localParse(t.start);if(isNaN(start))return;const at=start.getTime()-mins*60000;if(at<=now&&at>now-15*60000&&!seen.has(String(t.id))){notify('VVKBD reminder',`${t.title||'Task'} starts at ${start.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}`,String(t.id));seen.add(String(t.id))}});localStorage.setItem(seenKey,JSON.stringify([...seen]))}
...
window.addEventListener('visibilitychange',()=>{if(!document.hidden)scheduleNotifications()});
})();